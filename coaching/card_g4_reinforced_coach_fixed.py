#!/usr/bin/env python3
"""
Card G4 Reinforced Coaching System - FIXED VERSION
Implements intelligent coaching with Gemini 2.5 Pro meta-coaching
Fixed: Null checks, error handling, and performance optimizations
"""
...
class Card_G4_ReinforcedCoach:
    ...
    def get_historical_context(self, agent_id: str, doc_id: str) -> Dict:
        ...
        return {
            ...
            'trend': trend,
            'golden_examples': golden_examples if golden_examples else [],
    ...
    def _build_gemini_coaching_prompt(self, 
        ...
        # Safe access to best_ever accuracy
        best_accuracy = best_ever.get('accuracy', 0) if best_ever else 0
        
        # Calculate recent average safely
        recent_avg = 0
        if recent_runs:
            recent_accuracies = [r.get('accuracy', 0) for r in recent_runs if r]
...
HISTORICAL CONTEXT:
- Best Ever Accuracy: {best_accuracy:.2%}
- Recent Average: {recent_avg:.2%}
...
"""
    ...
    def _calculate_recall(self, extraction: Dict, ground_truth: Dict) -> float:
        ...
        found = sum(1 for k in ground_truth 
                   if extraction and k in extraction and extraction[k] == ground_truth[k])
...
def main():
    ...
    print("CARD G4 REINFORCED COACH TEST (FIXED VERSION)")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n📊 Testing None extraction handling...")
        perf_none = coach.analyze_performance(None, {'field': 'value'})
        print(f"  Accuracy: {perf_none.accuracy:.2%}")
        print(f"  Errors: {perf_none.errors}")
